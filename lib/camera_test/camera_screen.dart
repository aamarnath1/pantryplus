...
Future<void> _scanImage() async{
  ...
  try{
    ...
    final prompt = TextPart("You are a grocery analysis bot who is capable of processing images of grocery items and detailed receipts. You should first identify all items in the image or text and categorise them into proper types of food. Compulsarily Ignore non-food items. For each food items provide a json format which contains of type, name, calories, expiry_date & Category all five fields are compulsory it is required to be provided for each response, include all the items in a list, your response should be a list of Json objects. Do not miss a single item in the receipt or the captured image, if same items are repeated consider only one item and ignore repeated items. Provide exact type of the item captured, name, calories,  Expiry date can be in days/weeks/months(to calculate expiry date analyse the item and research in your database and get the estimated/ probable expiration date).category is always 3 option based on items classify them into 3 categories namely pantry, fridge & freezer.category is a always a required field provide category for individual items in every response. calories should always be just the numbers in kcal format no unit should be mentioned for calories, for ex: calories: 20, this response is enough instead of calories: 20kcal/calories: 20serving/kcal these responses are not required.You should ignore non-food items completely and not perform any analysis on them.Employ convolutional neural networks for image recognition, optical character recognition for processing receipt texts, and integrate a database for fetching food-related data. Your response should be in an array of json objects format. For example: [{type: fruit, name: orange, calories:20, expiry_date:1-2 weeks, category: pantry/fridge/freezer},{},{}……]. Strictly do not include ```json ``` in every response you provide and also ignore non edible items do not provide any analysis on them and do not include in the list.");
